...
import numpy as np   
import pandas as pd
from plumy.bouts_jbc import EWMABoutCounter as EWMABoutCounter_jbc
...
class wsn:
    ...
    def _computeGasMap(self, mapType, timeframe, bout_ampl=0):   
        t_idx = self._getTimeIndex(timeframe)
        t_s = self.t_s[t_idx]
        c_ppm = self.c_ppm[t_idx, :]
        print(str(t_s[0]) + ", " + str(t_s[-1]))
        ...
        for i in range(self.nr_of_sensors):
            if mapType is 'mean':
                ...
                # bout count    
                ebc_jbc=EWMABoutCounter_jbc(c_ppm[:,i], 2.5, 2.5)
                filtered_bouts, amps, durs = ebc_jbc.compute_original_bouts(ampthresh=bout_ampl, durthresh=0)
    ...
    def _plotGasMapSlice(self, fig, ax, gmap_slice, xlbl = True, ylbl = True, 
        ...
        if cb_lbl is not None:
            cax = fig.add_axes([0.77, 0.1, 0.03, 0.79])
            cb = fig.colorbar(im_map, cax=cax) 
            cb.set_label(cb_lbl)
            
        return im_map
    
    def plotGasMap(self, ax=None, map_type='mean', timeframe=None, interp='spline36', 
                   cm = 'YlOrRd', norm=None, clims=None):
        
        gmap = self._computeGasMap(map_type, timeframe) 
        
        cb_lbl = {'mean': 'ppm', 'var': 'ppm^2', 'max': 'ppm'}
        
        if not ax or ax is None:
            fig,ax = plt.subplots(3,1, figsize=(3.0, 10.0), dpi=150)
        else:
            fig = plt.gcf()
        ...
        for z in range(3):            
            im = self._plotGasMapSlice(fig, ax[z], gmap[z,:,:], 
                                       clims=clims, cm=cm, norm=norm)
            ax[z].text(-1.0, 1.0, 'Z = {0:.2f} m'.format(z_real[z]), 
                       rotation=90) 
